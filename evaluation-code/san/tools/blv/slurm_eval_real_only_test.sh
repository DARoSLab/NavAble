#!/bin/bash
#SBATCH --job-name=blv_eval_ronly
...
#
# Run real_v2-only test eval on a Stage-1Ronly or Stage-2RonlyC checkpoint.
# Uses *_finetune_real_only.py config (test_dataloader = real_v2/test only).
#
# Usage:
#   sbatch tools/blv/slurm_eval_real_only_test.sh MODEL CHECKPOINT [WORK_DIR]
#
# MODEL: mask2former | segformer | san
# CHECKPOINT: path to .pth (Stage-1Ronly or Stage-2RonlyC best)
# WORK_DIR: optional; defaults to work_dirs/eval_real_only/<model>_<ts>/
set -euo pipefail
...
MODEL="${1:?Usage: slurm_eval_real_only_test.sh MODEL CHECKPOINT [WORK_DIR]}"
CKPT="${2:?Usage: slurm_eval_real_only_test.sh MODEL CHECKPOINT [WORK_DIR]}"

CONFIG="${PROJECT_ROOT}/configs/blv/${MODEL}_finetune_real_only.py"
if [ ! -f "${CONFIG}" ]; then
...
WORK_DIR="${3:-${PROJECT_ROOT}/work_dirs/eval_real_only/${MODEL}_${SPLIT}_${DT}}"
mkdir -p "${WORK_DIR}"
...
bash "${MMSEG_ROOT}/tools/dist_test.sh" "${CONFIG}" "${CKPT}" 1 \
    ...
    --cfg-options \
        ...
        test_dataloader.dataset.data_prefix.img_path="img_dir/${SPLIT}" \
        test_dataloader.dataset.data_prefix.seg_map_path="ann_dir/${SPLIT}"
