...
#
# Real_final/test evaluation for the Final_results_and_checkpoints pipeline.
# - W&B run name: test-<arch>-final-<letter>-<data> (scalars only, no image upload)
# - Local PNG visualizations: ~10% sample saved to <work_dir>/vis/
# - Honest BLVMetric (excluded_class_indices=[10] turnstile, post-fix code)
#
# Usage:
#   sbatch Final_results_and_checkpoints/tools/slurm_eval_final.sh MODEL CONFIG CHECKPOINT
#
# Example:
#   sbatch Final_results_and_checkpoints/tools/slurm_eval_final.sh \
#     segformer C \
#     Final_results_and_checkpoints/work_dirs/segformer_finalC_real_synth01_<DT>/best_mIoU_iter_<N>.pth
set -euo pipefail
...
MODEL="${1:?Usage: slurm_eval_final.sh MODEL CONFIG CHECKPOINT}"
CONFIG="${2:?Usage: slurm_eval_final.sh MODEL CONFIG CHECKPOINT}"
CKPT="${3:?Usage: slurm_eval_final.sh MODEL CONFIG CHECKPOINT}"
...
# - Replace WandbVisBackend with WandbMetricsOnlyBackend (scalars only, no image upload)
# - Set W&B run name + tags
# - SegVisualizationHook fires every 10 test images (~148 PNGs of 1,482) saved locally
...
# Visualization hook: draw every 10th test image (~10% sample) → ~148 PNGs
...
echo "Vis sample  : every 10th test image (~148 PNGs) → ${WORK_DIR}/vis/"
