"""SegFormer finetune on opensrc V2 (12-class) data.

Strategy: replicate the validated old SegFormer setup exactly. Old data had
bg as ignore(255) and used ``RemapSegLabel(255→10)`` to turn it into a
trainable extra channel. New data has bg as class 0; we use the V2Fg dataset
which applies ``reduce_zero_label=True`` (bg(0)→255, fg shifts down by 1),
then ``RemapSegLabel(255→11)`` to land bg as the *last* trainable channel.

Net result: model has num_classes=12 with bg at index 11 (matches old layout
shifted by class count), metric uses num_classes=11 +
``segformer_extra_channel_fg_only=True`` to strip bg before argmax — the
exact same code path that produced 90.07 mIoU on synthetic.
"""
...
NUM_FG = 11
BG_CLASS_ID = NUM_FG  # 11 — bg is the last trainable channel
...
param_scheduler = [
    ...
    dict(type='PolyLR', power=1.0, by_epoch=False, begin=500, end=40000, eta_min=0.0),
]
...
model = dict(
    ...
    decode_head=dict(
        num_classes=NUM_FG + 1,  # 11 fg + 1 bg (at index 11)
        loss_decode=dict(
            type='CrossEntropyLoss',
            use_sigmoid=False,
            loss_weight=1.0,
            class_weight=[1.0] * NUM_FG + [0.15],  # fg=1.0, bg=0.15
        ),
    ),
)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-1-opensrc-segformer',
            tags=['finetune', 'opensrc', 'stage-1', 'segformer', 'v2-schema'],
        ),
    ),
]
