"""SegFormer Stage-2RonlyC: real_v2-only finetune from Stage-2A synth ckpt (no opensrc).

- ADE20k pretrained init
- 10,000 iters (real_v2 ~3.5k → ~28 epochs at batch 10)
- batch 10 (was 8), val every 500
"""
...
param_scheduler = [
    ...
    dict(type='PolyLR', power=1.0, by_epoch=False, begin=300, end=10000, eta_min=0.0),
]

train_cfg = dict(type='IterBasedTrainLoop', max_iters=10000, val_begin=500, val_interval=500)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-2Ronly-c-segformer',
            tags=['stage-2Ronly-c', 'real-only-from-synth', 'segformer', 'v2-schema'],
        ),
    ),
]
