"""Mask2Former Stage-2C-LONG: real fine-tune from Stage-2A synth-pretrained ckpt.

Long version of mask2former_finetune_real_synth_v2.py:
- iters bumped 7k → 45k (= Stage-1)
- LR restored 5e-6 → 1e-5 (= Stage-1, not halved)
- batch 4 (Stage-1 default — batch=8 + AMP triggered NaN loss_mask, see bugs.md)
- val cadence 1k → 2k (still tight enough to catch peak)
"""
...
# Stage-1 LR (not halved) — synth init has more to overcome than Stage-1.
optim_wrapper = dict(
    _delete_=True,
...
train_cfg = dict(type='IterBasedTrainLoop', max_iters=45000, val_begin=2000, val_interval=2000)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-2c-long-real-from-synth-mask2former',
            tags=['stage-2c-long', 'real-finetune-from-synth', 'mask2former', 'v2-schema'],
        ),
    ),
]
