"""Mask2Former Stage-2RonlyC: real_v2-only finetune from Stage-2A synth ckpt (no opensrc).

Same architecture/loss/pipeline as mask2former_finetune_real.py, but trains and
evaluates on real_v2 alone (opensrc dropped — its quality is questionable).
- Stage-2A synth pretrained init (load_from)
- 8,000 iters (real_v2/train ~3.5k → ~9 epochs at batch 4)
- batch 4 (Stage-1 default — batch=8 + AMP triggered NaN loss_mask, see bugs.md)
- val every 500
"""
...
param_scheduler = [
    ...
    dict(type='PolyLR', power=0.9, by_epoch=False, begin=300, end=8000, eta_min=0.0),
]

train_cfg = dict(type='IterBasedTrainLoop', max_iters=8000, val_begin=500, val_interval=500)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-2Ronly-c-mask2former',
            tags=['stage-2Ronly-c', 'real-only-from-synth', 'mask2former', 'v2-schema'],
        ),
    ),
]
