"""SAN Stage-2RonlyC: real_v2-only finetune from Stage-2A synth ckpt (no opensrc).

- COCO-stuff pretrained init (CLIP-based)
- 5,000 iters (real_v2 ~3.5k → ~34 epochs at batch 24)
- batch 24 (was 16), val every 500
"""
...
param_scheduler = [
    dict(type='PolyLR', power=1.0, by_epoch=False, begin=0, end=5000, eta_min=0.0),
]

train_cfg = dict(type='IterBasedTrainLoop', max_iters=5000, val_begin=500, val_interval=500)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-2Ronly-c-san',
            tags=['stage-2Ronly-c', 'real-only-from-synth', 'san', 'v2-schema'],
        ),
    ),
]
