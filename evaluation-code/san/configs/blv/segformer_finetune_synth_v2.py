"""SegFormer Stage-2A: synthetic-only pretrain on full V2 synth dataset.

Same dataset strategy as mask2former_finetune_synth_v2.py. Uses BLVDatasetV2Fg
(reduce_zero_label=True) and the RemapSegLabel(255→11) trick to land bg as the
last trainable channel — same validated path as segformer_finetune_real.
"""
...
# 35k iters @ batch=8 ≈ 1.55 epochs over 180,886 train images.
param_scheduler = [
    dict(type='LinearLR', start_factor=1e-6, by_epoch=False, begin=0, end=500),
    dict(type='PolyLR', power=1.0, by_epoch=False, begin=500, end=35000, eta_min=0.0),
]

train_cfg = dict(type='IterBasedTrainLoop', max_iters=35000, val_begin=5000, val_interval=5000)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-2a-synth-segformer',
            tags=['stage-2a', 'synth-pretrain', 'segformer', 'v2-schema'],
        ),
    ),
]
