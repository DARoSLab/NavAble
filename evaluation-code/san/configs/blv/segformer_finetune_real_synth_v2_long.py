"""SegFormer Stage-2C-LONG: real fine-tune from Stage-2A synth-pretrained ckpt.

- iters bumped 9k → 60k (= Stage-1)
- LR restored 5e-6 → 1e-5 (= Stage-1)
- batch 8 → 10 (better L40s utilization, ~30GB peak est., safe margin)
- val cadence 1k → 2k
"""
...
load_from = _os.environ.get(
    'BLV_STAGE2A_CKPT',
    _os.path.join(_blv_root, 'checkpoints', 'stage2a_best', 'segformer_synth_best.pth'),
)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-2c-long-real-from-synth-segformer',
            tags=['stage-2c-long', 'real-finetune-from-synth', 'segformer', 'v2-schema'],
        ),
    ),
]
