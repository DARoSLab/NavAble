"""SAN Stage-2A: synthetic-only pretrain on full V2 synth dataset, fg-only mode.

Same dataset strategy as mask2former_finetune_synth_v2.py.
"""
...
# 20k iters @ batch=16 ≈ 1.77 epochs over 180,886 train images.
param_scheduler = [
    dict(type='PolyLR', power=1.0, by_epoch=False, begin=0, end=20000, eta_min=0.0),
]

train_cfg = dict(type='IterBasedTrainLoop', max_iters=20000, val_begin=2500, val_interval=2500)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-2a-synth-san',
            tags=['stage-2a', 'synth-pretrain', 'san', 'v2-schema'],
        ),
    ),
]
