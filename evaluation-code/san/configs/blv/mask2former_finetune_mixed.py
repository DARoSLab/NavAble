"""Mask2Former Stage-2D: mixed training on real_v2 + opensrc + synth_v2 (subsampled).

Strategy: instead of two-stage synth-pretrain → real-finetune (which underperformed),
train all three datasets together from ADE init with real-weighted sampling so the
model never drifts to a synth-only manifold.

Per-batch composition (effective ratio):
  real_v2   × 40  → 140,920 (~39%)
  opensrc   ×  1  →  36,049 (~10%)
  synth_v2  ×  1  → 180,886 (~51%, full set)

- ADE20k init (NOT Stage-2A synth ckpt — start fresh, let mixed signal drive)
- 60,000 iters (= Stage-1 SegFormer length, more data → more iters needed than M2F's 45k)
- batch 4 (Stage-1 default — batch=8 + AMP triggered NaN loss_mask, see bugs.md), lr 1e-5
"""
...
synth_root = _os.path.join(_blv_root, 'data', 'synthetic_v2')
...
param_scheduler = [
    ...
    dict(type='PolyLR', power=0.9, by_epoch=False, begin=500, end=60000, eta_min=0.0),
]

train_cfg = dict(type='IterBasedTrainLoop', max_iters=60000, val_begin=4000, val_interval=4000)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-2d-mixed-mask2former',
            tags=['stage-2d', 'mixed-real-opensrc-synth', 'mask2former', 'v2-schema'],
        ),
    ),
]
