"""SegFormer Stage-1 finetune on real-world data (real_v2 + opensrc).

Same dataset strategy as mask2former_finetune_real.py:
ConcatDataset(real_v2/train, opensrc/train) for training, real_v2/{val,test}
for validation/test. Uses BLVDatasetV2Fg (reduce_zero_label=True) and the
RemapSegLabel(255→11) trick to land bg as the last trainable channel — same
validated path as segformer_finetune_opensrc.
"""
...
param_scheduler = [
    ...
    dict(type='PolyLR', power=1.0, by_epoch=False, begin=500, end=60000, eta_min=0.0),
]
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-1-real-segformer',
            tags=['finetune', 'stage-1', 'real', 'segformer', 'v2-schema'],
        ),
    ),
]
