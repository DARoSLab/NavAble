...
data_root = _os.path.join(_blv_root, 'data', 'synthetic')

train_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(type='LoadAnnotations'),
    dict(type='RemapSegLabel', src_label=255, dst_label=10),
    dict(type='RandomResize', scale=(2048, 640), ratio_range=(0.5, 2.0), keep_ratio=True),
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-1-synthetic-only-segformer',
            tags=['finetune', 'synthetic-only', 'stage-1', 'segformer'],
        ),
    ),
]
