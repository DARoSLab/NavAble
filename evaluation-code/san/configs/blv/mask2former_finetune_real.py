"""Mask2Former Stage-1 finetune on real-world data.

Train = ConcatDataset(real_v2/train, opensrc/train).
Val   = real_v2/val.
Test  = real_v2/test.

Both datasets share the V2 12-class schema (bg=0, 1..11 fg). BLVDatasetV2Fg
applies reduce_zero_label: bg(0)→255 ignore, fg(1..11)→0..10. The model trains
with num_classes=11 + matcher no-object slot (validated path).
"""
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-1-real-mask2former',
            tags=['finetune', 'stage-1', 'real', 'mask2former', 'v2-schema'],
        ),
    ),
]
