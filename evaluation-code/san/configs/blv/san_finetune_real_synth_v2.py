"""SAN Stage-2C: real fine-tune from Stage-2A synth-pretrained ckpt, fg-only mode."""
...
# Half of Stage-1 SAN LR (1e-4 → 5e-5).
optim_wrapper = dict(
    _delete_=True,
...
# 4500 iters ≈ 15% of Stage-1's 30k.
param_scheduler = [
    dict(type='PolyLR', power=1.0, by_epoch=False, begin=0, end=4500, eta_min=0.0),
]

train_cfg = dict(type='IterBasedTrainLoop', max_iters=4500, val_begin=500, val_interval=500)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-2c-real-from-synth-san',
            tags=['stage-2c', 'real-finetune-from-synth', 'san', 'v2-schema'],
        ),
    ),
]
