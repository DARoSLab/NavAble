"""Mask2Former finetune on opensrc V2 (12-class) data, fg-only mode.

Strategy: use ``BLVDatasetV2Fg`` which applies ``reduce_zero_label=True`` so
bg(0) → 255 (ignore) and classes 1..11 → 0..10. Trains exactly like the
validated old synthetic pipeline (num_classes=11, matcher handles no-object).

Opensrc images are 640×640, so crop_size is reduced from 1024 → 640.
"""
...
param_scheduler = [
    ...
    dict(type='PolyLR', power=0.9, by_epoch=False, begin=500, end=30000, eta_min=0.0),
]

train_cfg = dict(type='IterBasedTrainLoop', max_iters=30000, val_begin=999999, val_interval=2000)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='blv-seg',
            name='stage-1-opensrc-mask2former',
            tags=['finetune', 'opensrc', 'stage-1', 'mask2former', 'v2-schema'],
        ),
    ),
]
