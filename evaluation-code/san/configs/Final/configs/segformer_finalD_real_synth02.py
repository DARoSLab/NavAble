"""SegFormer Final-D: real ×12 + synth_0.2 mixed training, real-only eval. Scaling experiment 2.

Tests whether more synth data + matched real-duplication scales better than C.

- ADE20k pretrained init.
- Train: ConcatDataset(RepeatDataset(real_final, ×12) + synth_0_2)
         = 44,436 (real ×12) + 41,022 (synth_0.2) = 85,458 imgs/epoch (~52% real / ~48% synth).
- Val:   data/real_final/val.
- Test:  data/real_final/test.
- 200,000 iters at batch 10 (~24 epochs over 85K, ~32h budget at ~6.3K iters/h).
"""
...
synth_root = _os.path.join(_blv_root, 'data', 'synth_0_2')
...
param_scheduler = [
    ...
    dict(type='PolyLR', power=1.0, by_epoch=False, begin=1000, end=200000, eta_min=0.0),
]

train_cfg = dict(type='IterBasedTrainLoop', max_iters=200000, val_begin=10000, val_interval=10000)
...
default_hooks = dict(
    ...
    visualization=dict(type='SegVisualizationHook', draw=True, interval=10000),
)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            ...
            name='segformer-final-d-real-synth02',
            tags=['final', 'configD', 'segformer'],
        ),
    ),
]
