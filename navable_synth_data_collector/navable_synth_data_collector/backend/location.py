...
class LocationManager:
    ...
    def list_locations(self) -> List[str]:
        ...
        try:
            ...
            _log_warn(f"[NavAble] Failed to list locations: {exc}")
        return locations
    ...
    def create_location(
        ...
        _log_info(f"[NavAble] Created location '{name}' → {filepath}")
        return filepath
    ...
    def save_transform(
        ...
        with open(filepath, "w") as fh:
            json.dump(data, fh, indent=2)
        _log_info(f"[NavAble] Saved transform for location '{name}'")

    def delete_location(self, name: str) -> bool:
        """Remove the location directory and all its contents."""
        loc_dir = os.path.join(self._base_dir, name)
        if not os.path.isdir(loc_dir):
            _log_warn(f"[NavAble] Location directory not found: {loc_dir}")
            return False
        try:
            shutil.rmtree(loc_dir)
            _log_info(f"[NavAble] Deleted location '{name}' at {loc_dir}")
            if self._current_location == name:
                self._current_location = ""
            return True
        except OSError as exc:
            _log_error(f"[NavAble] Failed to delete location '{name}': {exc}")
            return False
