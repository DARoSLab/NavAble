...
class DataRecorder:
    ...
    def camera_path(self, path: str) -> None:
        if self._is_setup and path != self._camera_path:
            carb.log_warn(
                "[NavAble] Changing camera_path while the recorder is set up — "
    ...
    def setup(self, output_dir: str, rt_subframes: int = 4) -> None:
        ...
        if self._is_setup:
            carb.log_warn(
                "[NavAble] DataRecorder already set up — call teardown first."
            )
            return
        ...
        try:
            ...
            self._is_setup = True
            carb.log_info(
                f"[NavAble] DataRecorder setup complete — output={output_dir}, "
                f"resolution={self._resolution}, rt_subframes={self._rt_subframes}"
            )
        except Exception as exc:  # noqa: BLE001
            carb.log_error(f"[NavAble] DataRecorder setup failed: {exc}")
            self._release_resources()
            raise
    ...
    def reinitialize_writer(self, output_dir: str) -> None:
        ...
        if not self._is_setup or self._render_product is None:
            carb.log_warn(
                "[NavAble] reinitialize_writer called but recorder is not set up; "
        ...
        if self._writer is not None:
            try:
                ...
                carb.log_warn(f"[NavAble] Writer detach warning during reinit: {exc}")
            self._writer = None
        ...
        self._frame_count = 0
        carb.log_info(
            f"[NavAble] DataRecorder writer reinitialised → output={output_dir}"
        )

    async def capture_frame(self) -> None:
        """Capture one frame (async — must be awaited)."""
        if not self._is_setup:
            carb.log_error("[NavAble] Cannot capture — DataRecorder not set up.")
            return
        ...
        try:
            ...
            carb.log_error(
                f"[NavAble] capture_frame failed at frame {self._frame_count}: {exc}"
            )
    ...
    def teardown(self) -> None:
        ...
        if self._is_setup:
            carb.log_info(
                f"[NavAble] DataRecorder teardown — {self._frame_count} frames "
    ...
    async def prepare_for_stage_change_async(self) -> None:
        ...
        """
        carb.log_info("[NavAble] DataRecorder: prepare_for_stage_change_async — begin")

        # 1. Drain in-flight work.
        try:
            await rep.orchestrator.wait_until_complete_async()
            carb.log_info("[NavAble] DataRecorder: orchestrator drained")
        except Exception as exc:  # noqa: BLE001
            carb.log_warn(f"[NavAble] wait_until_complete_async warning: {exc}")

        # 2. Detach writer first.
        if self._writer is not None:
            try:
                self._writer.detach()
                carb.log_info("[NavAble] DataRecorder: writer detached")
            except Exception as exc:  # noqa: BLE001
                carb.log_warn(f"[NavAble] Writer detach warning (stage change): {exc}")
            self._writer = None
            ...
            try:
                ...
                if hydra_tex is not None:
                    try:
                        ...
                        carb.log_info("[NavAble] DataRecorder: hydra updates disabled")
                    except Exception as exc:  # noqa: BLE001
                        carb.log_warn(
                            f"[NavAble] set_updates_enabled(False) warning: {exc}"
                        )
            except Exception as exc:  # noqa: BLE001
                carb.log_warn(f"[NavAble] hydra_texture accessor warning: {exc}")

            try:
                self._render_product.destroy()
                carb.log_info("[NavAble] DataRecorder: render product destroyed")
            except Exception as exc:  # noqa: BLE001
                carb.log_warn(f"[NavAble] render_product.destroy warning: {exc}")
            self._render_product = None
        ...
        carb.log_info("[NavAble] DataRecorder: prepare_for_stage_change_async — done")
    ...
    def _release_resources(self, log_warnings: bool = False) -> None:
        if self._writer is not None:
            try:
                ...
                if log_warnings:
                    carb.log_warn(f"[NavAble] Writer detach warning: {exc}")
            self._writer = None
        ...
        if self._render_product is not None:
            try:
                ...
                if log_warnings:
                    carb.log_warn(f"[NavAble] Render product destroy warning: {exc}")
            self._render_product = None
