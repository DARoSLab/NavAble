    ...
    def _attach_stage_events(self) -> None:
        try:
            ...
            self._stage_sub = ctx.get_stage_event_stream().create_subscription_to_pop(
                self._on_stage_event, name="navable_synth_data_collector.stage_controller"
            )
        except Exception as exc:  # noqa: BLE001
            carb.log_warn(
                f"[NavAble] StageController: could not subscribe to stage events: {exc}"
            )
    ...
    async def switch_to(
        ...
        try:
            ...
            try:
                ...
                carb.log_warn(f"[NavAble] rep.orchestrator.stop warning: {exc}")
            ...
            for hook in list(self._pre_close_hooks):
                try:
                    ...
                    carb.log_error(f"[NavAble] pre-close hook raised: {exc}")
                    self._bus.emit(
            ...
            try:
                ...
                carb.log_warn(
                    f"[NavAble] wait_until_complete_async warning: {exc}"
                )
            ...
            try:
                ...
                carb.log_warn(f"[NavAble] close_stage_async warning: {exc}")
            ...
            carb.log_warn(
                f"[NavAble] StageController: opening {usd_path} "
                f"(timeout={timeout}s)"
            )
            ...
            try:
                ...
                carb.log_error(
                    f"[NavAble] open_stage_async TIMED OUT after "
                    f"{timeout}s: {usd_path}"
                )
                return False
            except Exception as exc:  # noqa: BLE001
                carb.log_error(f"[NavAble] open_stage_async threw: {exc}")
                return False
            carb.log_warn(
                f"[NavAble] StageController: opened {usd_path} in "
                f"{time.monotonic() - t_open:.1f}s"
            )
            if not ok:
                carb.log_error(
                    f"[NavAble] open_stage_async failed: {err}"
                )
                return False
            ...
            for hook in list(self._post_open_hooks):
                try:
                    ...
                    carb.log_error(f"[NavAble] post-open hook raised: {exc}")
                    self._bus.emit(
