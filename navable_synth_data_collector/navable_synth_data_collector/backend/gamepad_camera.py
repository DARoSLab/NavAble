...
class GamepadCameraController:
    ...
    def __init__(
        ...
        _ext = "exts.navable_synth_data_collector"
    ...
    def enable(self) -> None:
        if self._enabled:
            carb.log_warn("[NavAble] GamepadCameraController already enabled.")
            return
        ...
        self._update_sub = (
            ...
            .create_subscription_to_pop(
                self._on_update, name="navable_synth_data_collector.gamepad_camera"
            )
        )

        self._enabled = True
        carb.log_info(
            f"[NavAble] GamepadCameraController enabled  camera={self._camera_path}"
        )
    ...
    def disable(self) -> None:
        ...
        carb.log_info("[NavAble] GamepadCameraController disabled.")
            ...
            if stage is not None:
                ...
                if prim.IsValid():
                    try:
                        ...
                        carb.log_info(f"[NavAble] Camera focal length → {val} mm")
                    except Exception as exc:
                        carb.log_warn(f"[NavAble] Failed to update focal length: {exc}")
    ...
    def _ensure_camera_prim(self) -> None:
        ...
        try:
            ...
            carb.log_warn(f"[NavAble] Could not read stage metersPerUnit: {exc}")

        prim = stage.GetPrimAtPath(self._camera_path)
        if not prim.IsValid():
            cam = UsdGeom.Camera.Define(stage, self._camera_path)
            prim = cam.GetPrim()
            carb.log_info(f"[NavAble] Created camera prim at {self._camera_path}")
        else:
        ...
        if self._focal_length is not None and cam:
            try:
                ...
                carb.log_warn(f"[NavAble] Failed to set focal length: {exc}")
        ...
        if op_names != ["xformOp:translate", "xformOp:rotateZ", "xformOp:rotateX"]:
            ...
            carb.log_info(
                f"[NavAble] Configured Z-up rotation ops on {self._camera_path}"
            )
    ...
    def _on_gamepad_event(self, event, *args) -> bool:
        ...
        if inp == G.DPAD_UP and val > 0.5:
            ...
            carb.log_info(f"[NavAble] Move speed → {self._move_speed:.1f} m/s")
            ...
            carb.log_info(f"[NavAble] Slow mode {'ON' if self._slow_mode else 'OFF'}")
        elif inp == G.X and val > 0.5:
            if self.record_toggle_callback is not None:
                try:
                    self.record_toggle_callback()
                except Exception as exc:  # noqa: BLE001
                    carb.log_error(f"[NavAble] record_toggle_callback raised: {exc}")
