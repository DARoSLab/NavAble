...
class AssetBrowser:
        ...
        if self._bus is not None:
            self._tick_sub = (
                ...
                .create_subscription_to_pop(
                    self._on_tick, name="navable_synth_data_collector.asset_browser.tick"
                )
            )
    ...
    def set_folder(
        ...
        carb.log_info(
            f"[NavAble] AssetBrowser scanned '{folder_path}' — "
    ...
    def capture_transform_from_prim(self, prim_path: str) -> bool:
        ...
        if stage is None:
            carb.log_error("[NavAble] No USD stage available.")
            return False

        prim = stage.GetPrimAtPath(prim_path)
        if not prim.IsValid():
            carb.log_error(f"[NavAble] Prim at '{prim_path}' is invalid.")
            return False
        ...
        carb.log_info(
            f"[NavAble] Captured transform from {prim_path}: "
    ...
    def next_asset(self) -> bool:
        if not self._assets:
            carb.log_warn("[NavAble] No assets available — set a folder first.")
            return False
    ...
    def load_asset(self, index: int, preserve_transform: bool = True) -> bool:
        if index < 0 or index >= len(self._assets):
            carb.log_error(
                f"[NavAble] Asset index {index} out of range [0, {len(self._assets)})."
            )
            return False
        ...
        try:
            ...
            carb.log_warn(f"[NavAble] Could not clear viewport selection: {exc}")
        ...
        if self._current_prim_path:
            ...
            if existing.IsValid():
                try:
                    ...
                    carb.log_error(
                        f"[NavAble] Failed to delete prim {self._current_prim_path}: {exc}"
                    )
                    return False
        ...
        try:
            ...
            carb.log_error(f"[NavAble] Failed to create reference at {new_path}: {exc}")
            return False

        prim = stage.GetPrimAtPath(new_path)
        if not prim.IsValid():
            carb.log_error(f"[NavAble] New prim at {new_path} is invalid.")
            return False
        ...
        try:
            ...
            carb.log_warn(f"[NavAble] Could not set transform on {new_path}: {exc}")
        ...
        carb.log_info(
            f"[NavAble] Loaded asset [{index + 1}/{len(self._assets)}] "
    ...
    def _apply_semantic_label(
        ...
        try:
            ...
            carb.log_error(
                "[NavAble] Could not import isaacsim.core.utils.semantics — "
        ...
        for path in stale_paths:
            ...
            if old_prim.IsValid():
                try:
                    ...
                    carb.log_warn(f"[NavAble] Failed to remove label from {path}: {exc}")
        self._labeled_prim_paths.clear()
        ...
        if self._class_name:
            try:
                ...
                carb.log_info(
                    f"[NavAble] Semantic label '{self._class_name}' applied to "
                    f"{target_prim.GetPath()}"
                )
            except Exception as exc:  # noqa: BLE001
                carb.log_error(
                    f"[NavAble] Failed to add semantic label to "
