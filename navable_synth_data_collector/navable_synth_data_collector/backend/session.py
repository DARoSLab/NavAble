...
class Session:
    ...
    def _register_stage_hooks(self) -> None:
        ...
        async def ensure_camera_prim() -> None:
            # Ensure the camera prim exists on the freshly opened
            ...
            try:
                ...
                carb.log_warn(
                    f"[NavAble] Could not ensure camera prim post-stage-swap: {exc}"
                )
        ...
        async def reenable_gamepad() -> None:
            if self._gamepad_was_enabled_before_swap:
                try:
                    ...
                    carb.log_warn(
                        f"[NavAble] Could not re-enable gamepad post-stage-swap: {exc}"
                    )
    ...
    def stop_trajectory_recording(self) -> Optional[str]:
        ...
        try:
            ...
            carb.log_warn(f"[NavAble] trajectory_saved emit failed: {exc}")
        return path
    ...
    def _on_asset_moved(
        ...
        try:
            ...
            carb.log_warn(f"[NavAble] Auto-save location transform failed: {exc}")
        ...
        for asset_idx in range(total_assets):
            ...
            for traj_idx, traj_name in enumerate(traj_names):
                ...
                try:
                    ...
                    carb.log_error(f"[NavAble] record_all: {traj_path}: {exc}")
                    completed_units += 1
                    continue
    ...
    async def collect_all(
        ...
        try:
            for env_idx, plan in enumerate(plans):
                ...
                carb.log_warn(
                    f"[NavAble] collect_all: loading env "
                ...
                if not ok:
                    ...
                    carb.log_error(f"[NavAble] collect_all: {msg}")
                ...
                for loc_idx, (loc_name, traj_names) in enumerate(
                    ...
                    try:
                        ...
                        carb.log_warn(
                            f"[NavAble] collect_all: could not load "
                    ...
                    for asset_idx in range(total_assets):
                        ...
                        for traj_idx, traj_name in enumerate(traj_names):
                            ...
                            try:
                                ...
                                carb.log_error(
                                    f"[NavAble] collect_all: {traj_path}: {exc}"
                                )
                                completed_units += 1
                                continue
