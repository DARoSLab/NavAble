...
class TrajectoryRecorder:
    ...
    def start_recording(
        ...
        if self._recording:
            carb.log_warn("[NavAble] Recording already in progress — ignoring start.")
            return
        ...
        self._update_sub = (
            ...
            .create_subscription_to_pop(
                self._on_update, name="navable_synth_data_collector.trajectory_recorder"
            )
        )
        carb.log_info(f"[NavAble] Trajectory recording started — name='{name}'")
    ...
    def stop_recording(self) -> Dict[str, Any]:
        ...
        if was_recording:
            carb.log_info(
                f"[NavAble] Trajectory recording stopped — {self._frame_count} frames."
            )
        return data
    ...
    def save_trajectory(self, filepath: str) -> str:
        ...
        carb.log_info(f"[NavAble] Trajectory saved → {filepath}")
        return filepath
...
class TrajectoryPlayer:
    ...
    def load_trajectory(self, filepath: str) -> Dict[str, Any]:
        ...
        carb.log_info(
            f"[NavAble] Loaded trajectory '{self._trajectory.get('name', '?')}' "
    ...
    def play(self, on_complete: Optional[Callable[[], None]] = None) -> None:
        if self._trajectory is None or self.total_frames == 0:
            carb.log_warn("[NavAble] No trajectory loaded — cannot play.")
            return
        if self._playing:
            carb.log_warn("[NavAble] Playback already in progress.")
            return
        ...
        self._update_sub = (
            ...
            .get_update_event_stream()
            .create_subscription_to_pop(
                self._on_update, name="navable_synth_data_collector.trajectory_player"
            )
        )
        carb.log_info(
            f"[NavAble] Trajectory playback started — {self.total_frames} frames."
        )
    ...
    def stop(self, fire_on_complete: bool = False) -> None:
        ...
        if self._update_sub is not None:
            try:
                ...
                pass
            self._update_sub = None
        carb.log_info("[NavAble] Trajectory playback stopped.")
        ...
        if fire_on_complete and self._on_complete_callback is not None:
            ...
            try:
                ...
                carb.log_error(f"[NavAble] on_complete callback error: {exc}")
        ...
        if self._current_frame >= len(frames):
            ...
            if self._update_sub is not None:
                try:
                    ...
                    pass
                self._update_sub = None
            carb.log_info("[NavAble] Trajectory playback complete.")
...
class TrajectoryManager:
    ...
    def set_project_paths(
        ...
        carb.log_info(f"[NavAble] TrajectoryManager directory → {self._directory}")
        return self._directory
    ...
    def load(self, filename: str) -> Dict[str, Any]:
        ...
        carb.log_info(f"[NavAble] TrajectoryManager loaded {filepath}")
        return data

    def save(self, data: Dict[str, Any], filename: str) -> str:
        filepath = os.path.join(self._directory, filename)
        _io.write_trajectory_json(data, filepath)
        carb.log_info(f"[NavAble] TrajectoryManager saved {filepath}")
        return filepath
