"""Config loading for the NavAble Synthetic Data Extension.
...
2. Carb settings under ``/exts/navable_synth_data_collector/*`` (these come
...
class Defaults:
    ...
    # Multi-class CLI (`navable-collect collect-classes`).  Empty list means
...
def _default_yaml_path() -> str:
    ...
    Layout on disk::
        ...
        <ext_root>/navable_synth_data_collector/backend/config.py  ← __file__
    """
...
def _read_yaml(path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if carb is not None:
            carb.log_warn(f"[NavAble] Failed to parse config.yaml: {exc}")
        else:
...
_EXT_KEY = "exts.navable_synth_data_collector"
